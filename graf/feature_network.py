...
class upconv(nn.Module):
    ...
    def forward(self, x):  # torch.Size([32, 256, 6, 23])
        # performs the upsampling. 
        # The function nn.functional.interpolate is used to resize the input x to a larger size determined by the scale_factor. 
        # The parameter align_corners=True is typically used for the "linear", "bilinear", 
        # and "trilinear" modes of interpolation to better align the corners of the input and output. 
        # The mode="bilinear" indicates bilinear upsampling, which is a method of interpolation used for 2D grid data.

        # x: torch.Size([32, 256, 12, 46]), self.scale: 2
...
class ResUNet(nn.Module):
    ...
    def skipconnect(self, x1, x2):
        # x1: torch.Size([32, 128, 12, 45])
        # x2: torch.Size([32, 128, 12, 46])
        diffY = x2.size()[2] - x1.size()[2]  # 0
        diffX = x2.size()[3] - x1.size()[3]  # 1

        x1 = F.pad(x1, (diffX // 2, diffX - diffX // 2, diffY // 2, diffY - diffY // 2))  # torch.Size([32, 128, 12, 46])
